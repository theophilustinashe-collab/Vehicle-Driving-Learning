...
import {
  PlayCircle, Trophy, Target, Flame,
  ChevronRight, BarChart3, Clock, CheckCircle,
  Sparkles, Brain, Coins, Car, Crown, Star,
  Gauge, Zap, Award, Compass, Flag, Signpost, BookOpen,
  Calendar, ClipboardCheck, AlertCircle, ShieldCheck, Pencil
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: serverDashboard, isLoading: isServerLoading } = useGetDashboard();
  ...
  /** Flag determining if loading skeleton should be shown (optimistic rendering used) */
          ...
          <Link href="/settings" className="relative shrink-0 cursor-pointer group/avatar">
             <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-slate-900 border-4 border-white dark:border-slate-800 shadow-2xl flex items-center justify-center text-white font-black text-xl md:text-2xl relative overflow-hidden">
                {user?.avatarUrl && user.avatarUrl.trim() !== "" ? (
                  <img src={user.avatarUrl} alt={user.name || "User Avatar"} className="w-full h-full object-cover relative z-10 transition-transform duration-500 group-hover/avatar:scale-110" />
                ) : (
                  <>
                    <div className="absolute inset-0 bg-gradient-to-br from-primary via-primary/80 to-indigo-600 opacity-90" />
                    <span className="relative z-10 drop-shadow-md">{user?.name ? user.name.charAt(0).toUpperCase() : '?'}</span>
                  </>
                )}
                <motion.div
                  animate={{ x: ["-100%", "200%"] }}
                ...
                {!dashboard.recentTests || dashboard.recentTests.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-slate-800">
                    <AnimatePresence mode="popLayout">
                      {dashboard.recentTests.slice(0, 5).map((test: any) => (
                        <motion.div
                          key={test.sessionId}
                        ...
                        >
                          <Link href={`/test/${test.sessionId}/results`} className="absolute inset-0 z-20" />
                          ...
                          <div className="flex items-center gap-5 relative z-10">
                            <div className={cn(
                              ...
                              test.passed ? 'bg-emerald-50 text-emerald-600 border-emerald-100 shadow-emerald-500/10' : 'bg-red-50 text-red-600 border-red-100 dark:bg-red-500/10 dark:border-red-500/20 shadow-red-500/10'
                            )}>
                              {test.percentage}%
                            </div>
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center text-[10px] text-slate-400 font-bold uppercase tracking-widest gap-4">
                                <span className="flex items-center gap-1.5"><Calendar size={12} className="text-slate-300" /> {new Date(test.completedAt).toLocaleDateString()}</span>
                                <span className="opacity-20">•</span>
                                <span className="flex items-center gap-1.5"><CheckCircle size={12} className="text-slate-300" /> {test.score}/{test.total} Score</span>
                              </div>
                            </div>
                          </div>
            ...
            <Card className="border-0 shadow-2xl liquid-glass text-white rounded-[2rem] overflow-hidden relative group cursor-pointer active:scale-95 transition-all">
               ...
               <CardContent className="p-6 space-y-5 relative z-10">
                  <div className="flex items-center justify-between">
                     ...
                     <Badge className="bg-white/10 text-white border-white/10 font-black text-[7px] uppercase tracking-widest px-2 py-1 backdrop-blur-md">Premium Assets</Badge>
                  </div>
                  <div>
                     <h3 className="text-xl font-black tracking-tighter uppercase leading-none">The Garage</h3>
                     <p className="text-slate-400 font-bold text-[10px] leading-tight uppercase tracking-wider mt-2 opacity-70">Unlock legendary SADC vehicles and profile prestige.</p>
                  </div>
             ...
             <Card className="border-0 shadow-sm ring-1 ring-slate-100 dark:ring-slate-800 rounded-[2rem] overflow-hidden bg-white dark:bg-slate-900">
               ...
               <CardContent className="p-4">
                         ...
                         <div className="flex items-center gap-3 min-w-0 relative z-10">
                            <div className="relative">
                               <div className={cn(
                                 ...
                                 idx === 0 ? "bg-amber-400 text-white" : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white"
                               )}>
                                  {player.avatarUrl ? <img src={player.avatarUrl} alt={player.name} className="w-full h-full object-cover" /> : (player.name ? player.name.charAt(0) : '?')}
                               </div>
                               {idx === 0 && <Crown className="absolute -top-1.5 -right-1.5 w-4 h-4 text-amber-500 drop-shadow-md" />}
                            </div>
                            <div className="min-w-0">
                               <p className="font-black text-slate-900 dark:text-white uppercase text-[10px] truncate">{player.name}</p>
                               <div className="flex items-center gap-1.5 mt-0.5">
                                  <Badge className="bg-primary/10 text-primary text-[6px] font-black h-3.5 px-1 border-0">LV {player.level}</Badge>
                                  <span className="text-[8px] font-bold text-slate-400 uppercase tracking-tighter tabular-nums">{player.xp.toLocaleString()} XP</span>
                               </div>
                            </div>
                         </div>
                 ...
                 <Link href="/leaderboard" className="block mt-4">
                    <Button variant="ghost" className="w-full h-10 border-2 border-dashed rounded-xl font-black uppercase text-[8px] tracking-widest text-slate-400 hover:text-primary hover:border-primary transition-all group">
